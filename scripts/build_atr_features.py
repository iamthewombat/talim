#!/usr/bin/env python3
"""Build ATR feature parquet files from ingested broker bars."""
...
def main() -> int:
    ...
    for timeframe in args.timeframe:
        ...
        atr_col = f"atr_{args.period}"
        valid = int(final[atr_col].notna().sum())
        written.append(
            {
                "timeframe": timeframe,
                "path": str(output_path),
                "rows": len(final),
                "valid_atr_rows": valid,
    ...
    for item in written:
        print(
            ...
            f"({item['valid_atr_rows']} ATR values) to {item['path']} "
