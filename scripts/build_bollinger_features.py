#!/usr/bin/env python3
"""Build Bollinger Band feature parquet files from ingested broker bars."""
...
def main() -> int:
    ...
    suffix = f"{args.period}_{args.num_std:g}std".replace(".", "p")
    ...
    for timeframe in args.timeframe:
        ...
        features = build_bollinger_features(
            bars,
            period=args.period,
        ...
        middle_col = f"bb_middle_{suffix}"
        valid = int(final[middle_col].notna().sum())
        written.append(
            {
                "timeframe": timeframe,
    ...
    for item in written:
        print(
            ...
            f"({item['valid_bb_rows']} Bollinger values) to {item['path']} "
