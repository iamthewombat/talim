#!/usr/bin/env python3
"""Build MACD feature parquet files from ingested broker bars."""
...
def main() -> int:
    ...
    suffix = f"{args.fast_period}_{args.slow_period}_{args.signal_period}"
    ...
    for timeframe in args.timeframe:
        ...
        valid = int(final[macd_col].notna().sum())
        written.append(
            {
                "timeframe": timeframe,
                "path": str(output_path),
                "rows": len(final),
                "valid_macd_rows": valid,
    ...
    for item in written:
        print(
            ...
            f"({item['valid_macd_rows']} MACD values) to {item['path']} "
