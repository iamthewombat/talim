#!/usr/bin/env python3
"""Build RSI feature parquet files from ingested broker bars."""
...
def main() -> int:
    ...
    for timeframe in args.timeframe:
        ...
        rsi_col = f"rsi_{args.period}"
        valid = int(final[rsi_col].notna().sum())
        written.append(
            {
                "timeframe": timeframe,
                "path": str(output_path),
                "rows": len(final),
                "valid_rsi_rows": valid,
    ...
    for item in written:
        print(
            ...
            f"({item['valid_rsi_rows']} RSI values) to {item['path']} "
