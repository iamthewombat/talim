#!/usr/bin/env python3
"""Fetch IG historical bars for a canonical CFD instrument and write Parquet."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output",
        help="Output parquet path. Defaults to data/ig/<instrument>/<timeframe>.parquet",
    )
    parser.add_argument(
        "--append",
        action="store_true",
...
def _default_output(instrument: str, timeframe: str) -> Path:
    ...
    return Path("data") / "ig" / safe_instrument / f"{timeframe}.parquet"
...
def main() -> int:
    ...
    feed = IgPriceFeed.from_env(timeframe=args.timeframe)
    bars = feed.fetch_bars(args.instrument, page_size=args.bars)
