...
def _strategy_allowed_instruments(strategy_name: str) -> set[str] | None:
    """Infer the instruments a strategy is authored for from its name.

    Talim's current strategy modules are named with an instrument suffix
    (for example ``momentum-US500`` and ``momentum-AU200``). Until strategies
    expose first-class metadata, keep live scanning from applying a US500
    strategy to AU200 bars (or vice versa) by enforcing that suffix here.

    ``ES`` is retained as a US500 alias because several mock/e2e tests use it
    as the S&P-style instrument for ``momentum-US500``.
    """
    suffix = strategy_name.rsplit("-", 1)[-1].upper()
    if suffix == "US500":
        return {"US500.cash", "ES"}
    if suffix == "AU200":
        return {"AU200.cash"}
    return None


def _strategy_matches_instrument(strategy_name: str, instrument: str) -> bool:
    allowed = _strategy_allowed_instruments(strategy_name)
    return allowed is None or instrument in allowed


def _is_momentum_strategy(strategy_name: str) -> bool:
    return strategy_name.startswith("momentum-")


def _signal_timestamp_key(signal: Signal) -> str:
    ts = signal.timestamp
    return ts.isoformat() if hasattr(ts, "isoformat") else str(ts)


def _should_emit_signal(strategy_name: str, signal: Signal, latest_bar: OHLCVBar, regime: str | None) -> bool:
    """Return whether a strategy signal should become the operator HITL signal.

    WP-85 noise guardrails:
    - only alert on a signal generated by the latest scanned bar; old crosses in
    ...
    if signal.timestamp != latest_bar.timestamp:
        logger.info(
            "signal_scanner: skipping stale %s signal for %s from %s; latest bar is %s",
            strategy_name,
            signal.instrument,
            signal.timestamp,
    ...
    key = (strategy_name, signal.instrument)
    emitted = (signal.side, _signal_timestamp_key(signal))
    if _context._last_emitted_signal.get(key) == emitted:
        logger.info(
            "signal_scanner: suppressing duplicate %s %s signal for %s at %s",
            strategy_name,
            signal.side,
            signal.instrument,
            signal.timestamp,
    ...
    _context._last_emitted_signal[key] = emitted
    return True
...
def signal_scanner(state: TalimState) -> TalimState:
    ...
    # Scan every subscribed instrument. Earlier PoC code only scanned the
    # first instrument and then ran every active strategy against those bars,
    # which could produce a US500 strategy signal for AU200.
    ...
    for instrument in instruments:
        ...
        for name in active_names:
            if not _strategy_matches_instrument(name, instrument):
                continue
