"""Close-out push to Discord via webhook.

Fires immediately from the execute node when a position is closed, so the
operator sees the result without waiting for the next cron poll. Uses a
plain Discord webhook URL (no bot token required) read from
`TALIM_DISCORD_CLOSEOUT_WEBHOOK`. If the env var is unset, the helpers
...
logger = logging.getLogger("talim.discord.closeout")

_WEBHOOK_ENV = "TALIM_DISCORD_CLOSEOUT_WEBHOOK"
...
def post_closeout(event: CloseoutEvent, *, client: httpx.Client | None = None) -> bool:
    ...
    webhook isn't configured or the request fails — close-out posts must
    never break the execute path.
    """
    url = _webhook_url()
    if not url:
        return False

    content = format_closeout_message(event)
    ...
    try:
        ...
        logger.exception("close-out webhook request failed")
        return False

    if 200 <= resp.status_code < 300:
        return True
    logger.warning(
        "close-out webhook returned %s: %s", resp.status_code, resp.text[:200]
    )
    return False
