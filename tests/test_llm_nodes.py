...
class TestStrategyUpdate:
    ...
    def test_proposes_param_change(self):
        ...
        assert update["strategy_params"]["momentum-US500"]["ema_fast_period"] == 5
...
class TestConverse:
    def test_finds_strategy_reference(self):
        refs = _find_strategy_references(
            "what does momentum-US500 think?", ["momentum-US500", "mean-reversion-US500"]
        )
        assert refs == ["momentum-US500"]

    def test_finds_multiple_strategies(self):
        refs = _find_strategy_references(
            "compare Momentum-US500 and mean-reversion-US500",
            ["momentum-US500", "mean-reversion-US500"],
        )
        assert set(refs) == {"momentum-US500", "mean-reversion-US500"}
    ...
    def test_loads_referenced_strategy_into_active(self):
        update = converse({"last_user_message": "what does momentum-US500 think?"})
        assert "momentum-US500" in update.get("active_strategies", [])
